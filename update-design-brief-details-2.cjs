...
// 1. Update the Print Button Text
content = content.replace(
  'IMPRIMIR EXPEDIENTE TÉCNICO',
  'DESCARGAR EXPEDIENTE PDF COMPLETO'
);

// 2. Identify the Page 3 block
// We search for the specific header "FASE 3: INVENTARIO TÉCNICO" and replace the entire <div> container of Page 3.

const markerHeader = 'FASE 3: INVENTARIO TÉCNICO';
const startDivIndex = content.lastIndexOf('<div className="print-page', content.indexOf(markerHeader));
const endDivIndex = content.indexOf('PAGE 3 / 4', startDivIndex) + 50; // Capture some extra to be safe

if (startDivIndex === -1 || content.indexOf(markerHeader) === -1) {
  console.error("Markers for Page 3 not found in App.tsx!");
  process.exit(1);
}

// We also need to find the closing tag of that specific print-page div.
// It usually ends with PAGE 3 / 4 </div> </div>
const trueEndIndex = content.indexOf('</div>', content.indexOf('PAGE 3 / 4', startDivIndex)) + 6;
const finalEndIndex = content.indexOf('</div>', trueEndIndex) + 6;

const newPage3 = `{/* PAGE 3: Programa de Necesidades (DETAILED) */}
                                ...
                                return (
                                  <div key={item.name} className="flex flex-col gap-2">
                                    ...
                                    <div className="flex flex-wrap gap-2 items-center bg-white/60 p-2 rounded-lg border border-gray-100/50">
                                      ...
                                      {(item.name.includes('bedroom') || item.name === 'master_suite' || item.name === 'secondary_bedrooms' || item.name === 'guest_bedroom') && details.bed && (
                                      ...
                                      {(item.name.includes('kitchen')) && details.stove && (
...
const finalContent = content.substring(0, startDivIndex) + newPage3 + content.substring(finalEndIndex);
fs.writeFileSync(targetFile, finalContent, 'utf8');
console.log("Successfully updated Page 3 with detailed Inventario Técnico.");
