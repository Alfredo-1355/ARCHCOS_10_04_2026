...
function DesignBriefPreview({ program, language, t, onClose }: { program: ArchitecturalProgram, language: Language, t: (key: any) => string, onClose: () => void }) {
  ...
  return (
    <motion.div 
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-[#E5E7EB] overflow-y-auto overflow-x-hidden no-scrollbar text-black font-sans print:bg-white print:p-0"
    >
      <style>{\`
        @media print {
      ...
      <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b px-6 py-4 flex justify-between items-center print:hidden shadow-sm" style={{borderColor: \`\${themeBrandBlue}20\`}}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-full transition-all text-black">
            <X size={24} />
          </button>
        </div>
      </div>

      <div id="db-container" className="flex flex-col items-center gap-8 py-8 print:py-0 print:gap-0 w-full">
            ...
            <div className="flex flex-col gap-4">
               ...
               <div className="grid grid-cols-3 gap-8 px-4">
                 <div className="space-y-1">
                    ...
                    <p className="font-bold text-sm tracking-tight">{program.levels}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[9px] font-bold uppercase text-black/40">Ceiling Height</span>
                    <p className="font-bold text-sm tracking-tight leading-tight">{getCeilingValue()}</p>
                 </div>
                 <div className="space-y-1">
                    <span className="text-[9px] font-bold uppercase text-black/40">Basement/Foundation</span>
                    <p className="font-bold text-sm uppercase tracking-tight">{t(program.basement)}</p>
                 </div>
               </div>
            </div>
        ...
        <div className="print-page w-[8.5in] min-h-[11in] bg-white shadow-[0_0_15px_rgba(0,0,0,0.1)] relative p-[0.6in] flex flex-col border-t-[8px]" style={{borderColor: themeBrandGreen}}>
          ...
          <div className="flex flex-col flex-1 gap-6">
            ...
            {/* Puntos Críticos */}
            <div className="p-5 rounded-lg border" style={{backgroundColor: \`\${themeBrandGreen}08\`, borderColor: \`\${themeBrandGreen}40\`}}>
            ...
            <div className="mt-auto border-2 border-dashed rounded-lg p-5 flex flex-col" style={{borderColor: \`\${themeDarkBlue}30\`}}>
               <h3 className="text-[10px] font-bold uppercase tracking-widest mb-3 flex items-center gap-2" style={{color: \`\${themeDarkBlue}60\`}}>
                 <PenTool size={12}/> Notas del Arquitecto / Requisitos Especiales
               </h3>
               <div className="space-y-5 flex-1 mt-2">
                 <div className="border-b" style={{borderColor: \`\${themeDarkBlue}15\`}}></div>
                 <div className="border-b" style={{borderColor: \`\${themeDarkBlue}15\`}}></div>
                 <div className="border-b" style={{borderColor: \`\${themeDarkBlue}15\`}}></div>
               </div>
            </div>
        ...
        {/* ======================================= */}
        <div className="print-page w-[8.5in] min-h-[11in] bg-white shadow-[0_0_15px_rgba(0,0,0,0.1)] relative p-[0.5in] flex flex-col border-t-[8px]" style={{borderColor: themeBrandBlue}}>
          <header className="border-b-[3px] pb-3 mb-4 flex items-end justify-between shrink-0" style={{borderColor: themeBrandBlue}}>
            <div>
              <h4 className="text-[9px] font-bold uppercase tracking-widest text-black/50 mb-1">FASE 3: INVENTARIO DE ÁREAS</h4>
              <h1 className="text-2xl font-bold uppercase tracking-tighter leading-none text-black">
                Programa de Necesidades
              </h1>
            </div>
            <div className="text-right">
              <p className="font-bold text-sm" style={{color: themeBrandBlue}}>Total Interior Net: {areas.interiorNet.toFixed(0)} SqFt</p>
            </div>
          </header>
          ...
          <div className="flex-1 overflow-visible">
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              {['SOCIAL', 'PRIVADOS', 'SERVICIOS', 'EXTERIORES', 'OTROS'].map((cat) => {
                ...
                // Group by Subdivisions (Parent Key)
                ...
                return (
                  <div key={cat} className="space-y-2 col-span-2">
                    <div className="flex justify-between items-center border-b-[2px] pb-1 mt-2" style={{borderColor: \`\${headerColor}40\`}}>
                      <h3 className="text-xs font-bold uppercase tracking-widest flex items-center gap-1.5" style={{color: headerColor}}>
                        <Layout size={14}/> ZONA: {cat}
                      </h3>
                      <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded text-white" style={{backgroundColor: headerColor}}>Total: {totalCategoryArea.toFixed(0)} SqFt</span>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      {Object.entries(groupedByParent).map(([pKey, itemsInGroup]) => {
                        // If it's just 1 independent item, render it normally
                        if (itemsInGroup.length === 1 && !itemsInGroup[0].isParent && pKey === itemsInGroup[0].name) {
                          const item = itemsInGroup[0];
                          return (
                            <div key={item.name} className="bg-white border p-2.5 flex justify-between items-center shadow-sm" style={{borderColor: \`\${headerColor}30\`, borderLeftWidth: '3px', borderLeftColor: headerColor}}>
                              <div className="flex flex-col">
                                 <span className="font-bold text-[10px] uppercase text-black leading-tight">
                                    {MIN_DIMENSIONS[item.name]?.label?.[language] || item.name}
                                 </span>
                                 {item.qty > 1 && <span className="text-[8px] font-bold uppercase mt-0.5" style={{color: headerColor}}>Mult: {item.qty} espacios × {item.baseArea.toFixed(0)}</span>}
                                 <span className="text-[8px] text-black/60 font-mono mt-0.5">DIMS: {item.l}' × {item.w}'</span>
                              </div>
                              <div className="text-right pl-2">
                                 <span className="font-bold font-mono text-sm block leading-none" style={{color: headerColor}}>{item.area.toFixed(0)}</span>
                                 <span className="text-[7px] uppercase font-bold text-black/40 tracking-widest block mt-0.5">SqFt</span>
                              </div>
                            </div>
                          );
                        }

                        // Rendering a Group (Parent + Children)
                        const parentLabel = MIN_DIMENSIONS[pKey]?.label?.[language] || pKey;
                        const totalGroupArea = itemsInGroup.reduce((sum, i) => sum + i.area, 0);

                        return (
                          <div key={pKey} className="p-2.5 rounded border col-span-1" style={{backgroundColor: \`\${headerColor}08\`, borderColor: \`\${headerColor}20\`}}>
                            <div className="flex justify-between items-center mb-2 pb-1 border-b" style={{borderColor: \`\${headerColor}15\`}}>
                               <span className="font-bold text-[10px] uppercase" style={{color: themeDarkBlue}}>Suite / Setup: {parentLabel}</span>
                               <span className="font-mono text-[9px] font-bold" style={{color: headerColor}}>{totalGroupArea.toFixed(0)} SqFt</span>
                            </div>
                              ...
                              {itemsInGroup.map(item => (
                                <div key={item.name} className="flex justify-between items-center">
                                  <div className="flex flex-col">
                                    ...
                                    {item.qty > 1 && <span className="text-[8px] font-bold uppercase" style={{color: headerColor}}>x{item.qty} ({item.baseArea.toFixed(0)} Ea)</span>}
                                  </div>
                                  <div className="text-right flex items-center gap-2">
                                    <span className="text-[8px] text-black/60 font-mono">{item.l}' × {item.w}' =</span>
                                    <span className="font-bold font-mono text-xs block leading-none" style={{color: headerColor}}>{item.area.toFixed(0)}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
          ...
          <div className="mt-auto shrink-0 text-center pt-4 text-[8px] font-bold uppercase tracking-[0.2em] text-black/20">
             PAGE 3 / 4
          </div>
        </div>
...
console.log("Successfully refined DesignBriefPreview layout mapped strictly to US Letter sizes (8.5x11in), isolated print dialog, subdivisions added, notes added.");
